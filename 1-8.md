...
Ans: We wrote a loop to print for ten times with message "Noun verb subject:" which is declared as method below:
```java
 private static String createSentence() {
    return "Noun verb subject.";
  }
  ```
  Below logic is different from ProducerHello:
  ```java
   for (int i = 1; i <= 10; i++) {
